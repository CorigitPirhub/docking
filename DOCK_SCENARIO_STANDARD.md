...
DockBench-v1 不是“随便凑若干障碍图”，而是一个**机制完备（mechanism-complete）**的两车 docking benchmark。它要同时覆盖 5 类能力：
...
5. **车道约束下的协同微调**：在 `LC (Lane-Constrained)` 类别中，follower 的可行空间受车道宽度严格限制，必须显式验证“航向差增大 → Leader 倒车转弯/前后切换微调需求显著上升”的机制。

因此，DockBench-v1 的类别必须围绕上述 5 种机制来定义。
...
- 环境相关：
  ...
  - 车道相关（`LC` 必需、其它 family 可为 0 或空）：`corridor_width_min_m / bottleneck_ratio / corridor_turn_deg_max / branch_count / drivable_area_ratio / off_lane_shortcut_gap_m / leader_reverse_turn_required / leader_micro_adjust_budget_m`
...
## 6. 5 类 family / category：DockBench-v1 的正式场景家族
...
## 6.5 `LC` — Lane-Constrained（车道/走廊约束关键）

**定义**：车辆的可行规划空间被车道/走廊宽度严格限制，follower 不能通过大范围自由绕行来修正对接几何。`LC` 的核心目的，是显式放大“初始航向角差”对对接难度的影响，并检验 leader 是否具备**倒车转弯 / 前后切换 / 带转角微调**等协同微机动能力。

**关键机制信号**：
1. `corridor_width_min_m` 非零且较小，且 scene 中显式给出 `drivable corridor`；
2. `off_lane_shortcut_gap_m` 显著大于 0，说明若脱离车道约束会明显缩短路径；
3. `heading_diff_deg` 随难度增大而增大；
4. `leader_reverse_turn_required` 与 `leader_micro_adjust_budget_m` 随难度增大而增大；
5. `lane_fidelity = true`，即 follower 不能在正式约束下通过“脱离车道的自由空间捷径”完成任务。

**正式解释（避免把 `LC` 做成几何补丁）**：
- `LC` 的约束对象不是“整张图上的自由空间”，而是**车道图 / corridor geodesic**；
- 因此 `off_lane_shortcut_gap_m` 应定义为
  \[
  \Delta L_{lane}=L_{geodesic}^{corridor}-\|p_{goal}-p_{start}\|_2
  \]
  其中 `L_geodesic^{corridor}` 沿 corridor centerline / branch graph 计算，而不是简单靠 polygon 内外判定；
- 对 `LC`，`lane_fidelity=true` 的正式判据采用“**geodesic inflation + shortcut evidence**”联合标准：
  1. `off_lane_shortcut_gap_m >= 0.75m`；
  2. 且满足 `outside_ratio >= 0.18` **或** `L_geodesic^{corridor} / L_euclid >= 1.12`。

这样即使 bay / pocket 本身也属于 drivable area，仍能通过 corridor geodesic 明确地区分“合法 lane-follow 路径”与“违反 lane axis 假设的对角捷径”。

**与其它 family 的区别**：
- `SC` 关注遮挡/切换；
- `EC` 关注 cooperative staging 带来的可解域扩展；
- `LC` 关注“通行宽度受限 + 航向差增大 + leader 微调需求上升”的组合机制。

因此，`LC` 不应被当作 `SC/EC` 的附属标签，而应作为独立场景类别进入 frozen benchmark。
...
**`5 families × 3 difficulty levels = 15 scenario cells`**。

---

## 8. 固定数据集规模、比例与 split

## 8.1 官方推荐规模：`DockBench-v1 = 90` 个 concrete scenes
...
- `90 = 15 cells × 6` 在**覆盖、平衡、算力**之间是当前最合理的折中：五个机制 family 都能以完全均衡的方式进入 `tuning / test / challenge`，其中 `LC` 不再是附属标签，而是正式 benchmark category。
...
- `Stage-2`：必须使用 `60` 个 test scenes 全量批跑，并按 `CF / SC / FC / EC / LC` 分 family 统计。
...
- representative readiness 也是 Stage-0 的正式组成部分：`CF_L2 / SC_L2 / FC_L2 / EC_L2 / LC_L2` 必须来自 frozen `test` split 且可被 Stage-1 直接调用。
...
当前仓库已经把这条流程固定为：
1. `scripts/generate_dockbench_v1.py` 生成 `90` 个 concrete scenes；
...
4. Stage-1 / Stage-2 只允许读取 `data/dockbench_v1/` 的 frozen manifest 与 representative，其中 `LC` 必须作为独立 family/category 被单独统计。
...
其核心不是“我们也做了个数据集”，而是：
1. **机制完备 taxonomy**：`CF / SC / FC / EC / LC` 不是语义命名，而是与 docking 内部机制一一对应；
...
4. **整个数据集应有多少测试场景？**  
   当前推荐官方版为 `90` 个 concrete scenes，其中 `60` 个 test；如需 CI，可额外维护 mini split，但正式结论只认官方 frozen split。

5. **每类场景占比应是多少？**  
   采用 cell-balanced：`5 families × 3 difficulty` 完全均衡；family 各 `20%`，difficulty 各 `33.3%`。
...
4. 报告必须按 `CF / SC / FC / EC / LC` 五个 family / category 分别统计；
...
## 15. 当前经验验证状态（2026-03-09）

基于刷新后的 `DockBench-v1`（`90` scenes / `60` test）已完成：
1. representative 验证：`artifacts/p_minus1_stage1_lane_refresh/P_MINUS1_STAGE1_SUITE_REPORT.md`；
2. pre-fix full Stage-2：`artifacts/dockbench_stage2_lc_refresh/P_MINUS1_STAGE2_REPORT.md`；
3. `LC-CRBS` round-1 full Stage-2：`artifacts/dockbench_stage2_lc_crbs_round1_manual/P_MINUS1_STAGE2_REPORT.md`。

当前经验结论：
- 新 benchmark 的 `CF / SC / FC / EC / LC` 五类 family 已能在 frozen split 上稳定分离出不同机制；
- `LC` family 的机制放大作用已经成立：在当前 frozen `test` 上，未加专门 `LC` 设计时，`co_bcfd` 在 `LC` 子集上以 collision 为主；
- 引入 `LC-CRBS` 后，`LC` 子集已从“collision 主导”转为“deadlock / timeout 主导”，说明 benchmark taxonomy 与新模块的机制对齐是成立的；
- 但 `LC` success 仍未翻转，说明当前问题已经收敛到 **lane-constrained reciprocal plan 的可完成性**，而不再是“场景标签打错了”。

因此，本规范当前的角色不再只是“定义一个新 family”，而是已经成为**方法缺陷定位器**：它能稳定指出 Co-BCFD 在 `LC` 下缺的正是 corridor-compatible cooperative basin formation。
